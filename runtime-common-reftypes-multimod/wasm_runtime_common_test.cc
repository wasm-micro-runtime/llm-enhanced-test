...
TEST_F(wasm_runtime_common_test_suite, functions_on_wasm_module)
{
    ...
    wasm_module = wasm_runtime_load(wasm_file_buf, wasm_file_size, error_buf,
                                    sizeof(error_buf));
    ASSERT_NE(wasm_module, nullptr) << error_buf;
    wasm_module_inst = wasm_runtime_instantiate(
        wasm_module, stack_size, heap_size, error_buf, sizeof(error_buf));
    ASSERT_NE(wasm_module_inst, nullptr) << error_buf;
    exec_env = wasm_runtime_create_exec_env(wasm_module_inst, stack_size);
    ASSERT_NE(exec_env, nullptr);

    // Operations on exec_env.
    ASSERT_EQ(true, wasm_runtime_register_module_internal("test", wasm_module,
                                                          nullptr, 0, error_buf,
                                                          sizeof(error_buf)))
        << error_buf;
    ASSERT_NE(nullptr, wasm_runtime_find_module_registered("test"));
    ASSERT_EQ(wasm_module_inst, wasm_runtime_get_module_inst(exec_env));
    ASSERT_EQ(exec_env->attachment,
              wasm_runtime_get_function_attachment(exec_env));
    ASSERT_EQ(wasm_module, wasm_exec_env_get_module(exec_env));

    wasm_runtime_set_user_data(exec_env, (void *)user_data);
    ASSERT_EQ((void *)user_data, wasm_runtime_get_user_data(exec_env));
    ...
    /* on_timer_event takes 1 i32 parameter in main.wasm */
    ASSERT_EQ(true, wasm_runtime_call_wasm(exec_env, func, 1, argv));
    exception_test = wasm_runtime_get_exception(wasm_module_inst);
    
    ASSERT_EQ(true, wasm_runtime_call_wasm_a(exec_env, func, 0, nullptr, 1,
                                              arguments));
    exception_test = wasm_runtime_get_exception(wasm_module_inst);

    /* wasm_runtime_call_wasm_v expects raw varargs, not wasm_val_t*. Use the
       real function instance and validate both success and arg-count failure. */
    ASSERT_EQ(true, wasm_runtime_call_wasm_v(exec_env, func, 0, nullptr, 1,
                                             (uint32)0));
    ASSERT_EQ(false, wasm_runtime_call_wasm_v(exec_env, func, 0, nullptr, 0));
    ...
    ASSERT_EQ(true, wasm_runtime_create_exec_env_singleton(wasm_module_inst));
    ASSERT_NE(nullptr, wasm_runtime_get_exec_env_singleton(wasm_module_inst));

    wasm_runtime_set_exception(wasm_module_inst, str_test);
    sprintf(str_tmp, "%s%s", str_exception, str_test);
    ASSERT_EQ(0, strcmp(str_tmp, wasm_runtime_get_exception(wasm_module_inst)));
    wasm_runtime_clear_exception(wasm_module_inst);
    ASSERT_EQ(nullptr, wasm_runtime_get_exception(wasm_module_inst));

    wasm_runtime_set_custom_data(wasm_module_inst, (void *)user_data);
    ASSERT_EQ((void *)user_data,
              wasm_runtime_get_custom_data(wasm_module_inst));

    offset_tmp = wasm_runtime_module_malloc(wasm_module_inst, 10, &ptr_tmp);
    ASSERT_NE(0U, offset_tmp);
    ASSERT_EQ(true,
    ...
    ASSERT_EQ(offset_tmp,
              wasm_runtime_addr_native_to_app(wasm_module_inst, ptr_tmp));
    ASSERT_EQ(true, wasm_runtime_get_native_addr_range(
                        wasm_module_inst, (unsigned char *)ptr_tmp,
                        &p_native_start_addr, &p_native_end_addr));
    ASSERT_NE(0U, wasm_runtime_module_realloc(wasm_module_inst, offset_tmp, 100,
                                             &ptr_tmp));
    ...
    offset_tmp = wasm_runtime_module_dup_data(wasm_module_inst, str_test,
                                              sizeof(str_test));
    ASSERT_EQ(0, strcmp(str_test, (char *)wasm_runtime_addr_app_to_native(
    ...
    ASSERT_EQ(false, wasm_runtime_call_wasm(nullptr, func, 0, argv));
    wasm_runtime_set_exception(wasm_module_inst, str_test);
    ASSERT_EQ(false, wasm_runtime_call_wasm(exec_env, func, 0, argv));
    wasm_runtime_clear_exception(wasm_module_inst);

    /* Wrong arg count should fail */
    ASSERT_EQ(false, wasm_runtime_call_wasm_a(exec_env, func, 0, nullptr, 2,
                                              arguments));
    /* Correct arg count should succeed */
    ASSERT_EQ(true, wasm_runtime_call_wasm_a(exec_env, func, 0, nullptr, 1,
                                             arguments));
...
TEST_F(wasm_runtime_common_test_suite, functions_on_aot_module)
{
    ...
    EXPECT_EQ(true, wasm_runtime_call_wasm(exec_env, func, 0, argv));
    exception_test = wasm_runtime_get_exception(wasm_module_inst);

    EXPECT_EQ(true, wasm_runtime_call_wasm_a(exec_env, func, 0, nullptr, 1,
                                              arguments));
    exception_test = wasm_runtime_get_exception(wasm_module_inst);
    EXPECT_EQ(true, wasm_runtime_call_wasm_v(exec_env, func, 0, nullptr, 1,
                                              arguments));
    exception_test = wasm_runtime_get_exception(wasm_module_inst);
