;; v128_or_test.wat - WebAssembly Text Format for v128.or opcode testing

(module
  ;; Function: test_v128_or
  ;; Description: Performs v128.or operation on two input v128 vectors
  ;; Parameters: Eight i32 values representing two v128 inputs (4+4)
  ;; Returns: Four i32 values representing v128 result
  (func $test_v128_or (export "test_v128_or")
    ...
    ;; Apply v128.or bitwise OR operation
    (local.get $vector1)
    (local.get $vector2)
    (v128.or)
    (local.set $result)
  ...
  (func $test_v128_or_commutative (export "test_v128_or_commutative")
  ...
  ;; Function for testing identity and absorber properties
  (func $test_v128_or_identity (export "test_v128_or_identity")
    (param $test_type i32)
    (result i64)
    ...
    ;; Select identity or absorber test based on parameter
    (local.get $test_type)
    (if (result v128)
      (then
        ;; Test type 0: Identity test (OR with all zeros)
        (local.get $test_vector)
        (v128.const i64x2 0 0)
        (v128.or)
      )
      (else
        ;; Test type 1: Absorber test (OR with all ones)
        (local.get $test_vector)
  ...
  (func $test_v128_or_lane_patterns (export "test_v128_or_lane_patterns")
    ...
    (if
      (then
        ;; Pattern 0: Checkerboard (0x5555... | 0x3333... = 0x7777...)
  ...
  (func $test_v128_or_power_patterns (export "test_v128_or_power_patterns")
    (result i64 i64)
    (local $vector1 v128)
    (local $vector2 v128)
    (local $result v128)

    ;; Set power-of-2 patterns that create combined result when ORed
  ...
  (func $test_v128_or_associativity (export "test_v128_or_associativity")
  ...
  ;; Function for testing idempotent property (a | a = a)
  (func $test_v128_or_idempotent (export "test_v128_or_idempotent")
