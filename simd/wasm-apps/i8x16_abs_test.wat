(module
  ...
  ;; Tests critical boundary cases including INT8_MIN overflow scenario
  ;; Expected behavior: abs(-128) = -128 (overflow), abs(127) = 127, others normal
  ...
  ;; Tests mixed extreme values: INT8_MIN, INT8_MAX, and intermediate values
  ...
  (func $test_idempotent_property (param $input v128) (result v128)
    (local.get $input)
    (i8x16.abs)
    (i8x16.abs)  ;; Apply abs twice to verify idempotent property
  )
